...
const AddToCart = ({info,selectedVariation,isAddedToCart,count}) => {
    ...
    useEffect(() => {
        ...
        const fetchAddToCartApi = async () => {
          try {
            ...
            // Construct conversion API data for AddToCart
        ...
        const time = getFormattedEventDetails(response?.data?.currentTimestamp);
            ...
            const eventData = {
              ...
              content_ids: response?.data?.products?.map(product => product.sku) || [],
              ...
              value: response?.data?.products?.[0]?.price*count,
            ...
            console.error("Error fetching AddToCart API:", error);
          }
        };
    
        if ( isAddedToCart) {
          // Check for the add-to-cart condition
          fetchAddToCartApi();
        }
      }, [info, selectedVariation, fbclid, isAddedToCart]);
  return (
    <div>
